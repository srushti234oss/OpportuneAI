...
def tracker(request):
    ...
    saved = enrich(saved_opportunities.filter(status='saved'))
    applied = enrich(saved_opportunities.filter(status='applied'))
    accepted = enrich(saved_opportunities.filter(status='accepted'))
    rejected = enrich(saved_opportunities.filter(status='rejected'))

    # Summary stats
    total = saved_opportunities.count()
    deadlines_this_week = sum(
        1 for x in saved + applied
        if x['urgency'] in ('urgent', 'soon')
    )
...
def update_note(request, id):
    if request.method == 'POST':
        item = SavedOpportunity.objects.get(id=id, user=request.user)
...
def skillmap(request):
    from .ai_engine import get_full_skill_report
    profile = StudentProfile.objects.filter(user=request.user).first()
    report = {}
    if profile:
        report = get_full_skill_report(profile)
    return render(request, 'skillmap.html', {'report': report})
